...
Public Class GrooveMicrometer25
    ...
    Private ReadOnly nominalRangesExt As Decimal() = {0, 5, 10, 15, 20, 25}
    Private ReadOnly nominalRangesInt As Decimal() = {5, 10, 15, 20, 25}
    ...
    Private Sub Window_Loaded(sender As Object, e As RoutedEventArgs) Handles Me.Loaded
        ComboBox1.Text = "Groove Micrometer"
        LoadDynamicLC()
        LoadCalibrationMasters()

        ' Add handlers for External trial textboxes
        For Each r In nominalRangesExt
        ...
        ' Add handlers for Internal trial textboxes
        For Each r In nominalRangesInt
    ...
    Private Sub PopulateCalibrationData()
        Try
            Dim dt = MySqlCls.GetGrooveMicrometer25Data(TextBox1.Text.Trim(), TargetCycle)
            If dt.Rows.Count > 0 Then
                ...
                ' Populate External Trials
                For Each r In nominalRangesExt
                    SetTrialsFromDB(row, $"Ext{r}_", GetTextBoxesForRange(r, True))
                Next
                ' Populate Internal Trials
                For Each r In nominalRangesInt
                    SetTrialsFromDB(row, $"Int{r}_", GetTextBoxesForRange(r, False))
                Next
    ...
    Private Sub AdjustVisibilityBySize(sizeStr As String)
        Try
                ...
                For Each tb In tbs
                    ...
                    Else
                        If ComboBox5.SelectedItem IsNot Nothing Then
                            ...
                            If lbl IsNot Nothing Then UpdateObservationLabel(r, True)
                        Else
                            tb.IsEnabled = False
                            tb.Background = New SolidColorBrush(Color.FromRgb(241, 245, 249))
                ...
                For Each tb In tbs
                    ...
                    Else
                        If ComboBox5.SelectedItem IsNot Nothing Then
                            tb.IsEnabled = True
                            If String.IsNullOrWhiteSpace(tb.Text) Then tb.Background = Brushes.White
                            If lbl IsNot Nothing Then UpdateObservationLabel(r, False)
                        Else
                            tb.IsEnabled = False
                            tb.Background = New SolidColorBrush(Color.FromRgb(241, 245, 249))
                            If lbl IsNot Nothing Then lbl.Foreground = New SolidColorBrush(Color.FromRgb(51, 65, 85))
            ...
            If ComboBox5.SelectedItem IsNot Nothing Then
                Dim firstInRange = nominalRangesExt.FirstOrDefault(Function(r) IsNominalInRange(r))
                If firstInRange >= 0 Then EnableRange(firstInRange, True)
            End If
        Catch ex As Exception
        End Try
    End Sub
    ...
    Private Sub ObservationKeyDown(sender As Object, e As KeyEventArgs)
        If e.Key = Key.Enter Then
            ...
            Dim isExt = tb.Name.Contains("Ext")
            Dim nameParts = tb.Name.Split("_"c) ' e.g., txtExt0_1 -> [txtExt0, 1] OR txtInt5_1 -> [txtInt5, 1]
    ...
    Private Sub TrialTextChanged(sender As Object, e As TextChangedEventArgs)
        ...
        Dim isExt = txtBox.Name.Contains("Ext")
        Dim prefix = If(isExt, "Ext", "Int")
        Dim ulCol = $"{prefix}{nominal}_UL"
        Dim llCol = $"{prefix}{nominal}_LL"

        Try
            UpdateObservationLabel(nominal, isExt)
            CalculateTMU()
            UpdateSizeLockState()
            ...
            If Decimal.TryParse(txtBox.Text, val) Then
                ' DUAL MODE: User might type raw reading (5.001) OR error offset (0.001)
                ' For Groove Micrometer, we treat the input as a raw reading and calculate error
    ...
    Private Sub EnableNextRange(currentNominal As Decimal, isExt As Boolean)
        If isExt Then
            Dim idx = Array.IndexOf(nominalRangesExt, currentNominal)
            For i = idx + 1 To nominalRangesExt.Length - 1
                If IsNominalInRange(nominalRangesExt(i)) Then
                    EnableRange(nominalRangesExt(i), True)
                    Return
                End If
            Next
            ' External finished, start Internal
            For Each r In nominalRangesInt
            ...
            Next
        Else
            Dim idx = Array.IndexOf(nominalRangesInt, currentNominal)
            For i = idx + 1 To nominalRangesInt.Length - 1
                If IsNominalInRange(nominalRangesInt(i)) Then
                    EnableRange(nominalRangesInt(i), False)
                    Return
                End If
            Next
        End If
    ...
    Private Sub EnableRange(nominal As Decimal, isExt As Boolean)
        If ComboBox5.SelectedItem Is Nothing Then Return
        If Not IsNominalInRange(nominal) Then Return
        Dim tbs = GetTextBoxesForRange(nominal, isExt)
        For Each tb In tbs
            tb.IsEnabled = True
            If String.IsNullOrWhiteSpace(tb.Text) Then tb.Background = Brushes.White
        Next
    ...
    Private Function GetTextBoxesForRange(nominal As Decimal, isExt As Boolean) As List(Of TextBox)
        Dim list As New List(Of TextBox)
        Dim prefix = If(isExt, "txtExt", "txtInt")
        For i = 1 To 3
    ...
    Private Sub UpdateObservationLabel(nominal As Decimal, isExt As Boolean)
        Dim tbs = GetTextBoxesForRange(nominal, isExt)
        Dim count = tbs.Where(Function(t) Not String.IsNullOrWhiteSpace(t.Text)).Count()
        Dim prefix = If(isExt, "Ext", "Int")
        Dim lblName = $"Label{prefix}{nominal}"
        Dim lbl = DirectCast(Me.FindName(lblName), TextBlock)
    ...
    Private Sub LoadDynamicLC()
        Try
            Dim dt = MySqlCls.ReadDatatable("SELECT DISTINCT LC FROM groove_micrometer_25 WHERE RowType='MASTER' ORDER BY LC ASC")
    ...
    Private Sub ComboBox5_SelectionChanged(sender As Object, e As SelectionChangedEventArgs) Handles ComboBox5.SelectionChanged
        If ComboBox5.SelectedItem IsNot Nothing Then
            Dim selectedLC = ComboBox5.SelectedItem.ToString()
            _masterLimits = MySqlCls.GetGrooveMicrometer25MasterLimits(selectedLC)
            ResetAllTrials()
            AdjustVisibilityBySize(TextBox21.Text)
            EnableRange(nominalRangesExt(0), True)
        Else
    ...
    Private Sub EnvTextChanged(sender As Object, e As TextChangedEventArgs) Handles TextBox30.TextChanged, TextBox28.TextChanged, TextBox20.TextChanged
        ...
        Try
            ...
            If Decimal.TryParse(txtBox.Text, val) Then
                ...
                ElseIf txtBox.Name = "TextBox20" Then : llCol = "Depth_LL" : ulCol = "Depth_UL"
                End If
        ...
        For Each r In nominalRangesExt
            For Each tb In GetTextBoxesForRange(r, True)
                If tb.Background IsNot Nothing AndAlso TypeOf tb.Background Is SolidColorBrush Then
                    If DirectCast(tb.Background, SolidColorBrush).Color = Color.FromRgb(255, 182, 193) Then isNG = True : Exit For
                End If
            Next
    ...
    Private Sub Button2_Click(sender As Object, e As RoutedEventArgs) Handles Button2.Click
        ...
        For Each r In nominalRangesExt
            Dim tbs = GetTextBoxesForRange(r, True)
            Dim count = tbs.Where(Function(t) Not String.IsNullOrWhiteSpace(t.Text)).Count()
            If count > 0 AndAlso count < 3 Then MessageBox.Show($"Please complete all 3 trials for External {r}mm.") : Return
        Next
        For Each r In nominalRangesInt
            Dim tbs = GetTextBoxesForRange(r, False)
            Dim count = tbs.Where(Function(t) Not String.IsNullOrWhiteSpace(t.Text)).Count()
            If count > 0 AndAlso count < 3 Then MessageBox.Show($"Please complete all 3 trials for Internal {r}mm.") : Return
        Next
        ...
        ' Prepare arrays
        Dim ext0 = GetTrialsArrayFromUI(0, True), ext5 = GetTrialsArrayFromUI(5, True), ext10 = GetTrialsArrayFromUI(10, True)
        Dim ext15 = GetTrialsArrayFromUI(15, True), ext20 = GetTrialsArrayFromUI(20, True), ext25 = GetTrialsArrayFromUI(25, True)
        Dim int5 = GetTrialsArrayFromUI(5, False), int10 = GetTrialsArrayFromUI(10, False), int15 = GetTrialsArrayFromUI(15, False)
        Dim int20 = GetTrialsArrayFromUI(20, False), int25 = GetTrialsArrayFromUI(25, False)

        Dim saved As Boolean = False
        If IsEditMode Then
            saved = MySqlCls.UpdateGrooveMicrometer25(
                _instrumentName, TextBox1.Text.Trim(), TargetCycle, TextBox21.Text, ComboBox5.SelectedItem.ToString(), TextBoxColor.Text, ComboBox2.Text, TextBox30.Text, TextBox28.Text, TextBox29.Text,
                ext0, ext5, ext10, ext15, ext20, ext25,
                int5, int10, int15, int20, int25,
                ...
                calibDateToSave)
        Else
            saved = MySqlCls.InsertGrooveMicrometer25(
                _instrumentName, TextBox1.Text.Trim(), cycleName, TextBox21.Text, ComboBox5.SelectedItem.ToString(), TextBoxColor.Text, ComboBox2.Text, TextBox30.Text, TextBox28.Text, TextBox29.Text,
                ext0, ext5, ext10, ext15, ext20, ext25,
                int5, int10, int15, int20, int25,
                If(String.IsNullOrEmpty(TextBox20.Text), "-", TextBox20.Text), TextBox24.Text, TextBox26.Text, TextBox25.Text, statusVal, TextBox19.Text,
                calibDateToSave)
        ...
        If saved Then
            ...
            MySqlCls.UpdateRegularCalibrationStatus(TextBox1.Text.Trim(), calibDateToSave, statusVal, targetCyc)
            If Not String.IsNullOrEmpty(targetCyc) Then MySqlCls.InsertResultRecord(TextBox1.Text.Trim(), _category, _instrumentName, targetCyc)

            ' Save masters to cache
            CalibrationMasterCache.SaveMasters(TextBox1.Text.Trim(), targetCyc, _selectedMasters)

            If statusVal = "NG" Then
                ProcessNGFlow(TextBox1.Text.Trim(), targetCyc, calibDateToSave, statusVal)
            End If
    ...
    Private Function GetTrialsArrayFromUI(nominal As Decimal, isExt As Boolean) As Object()
        Dim tbs = GetTextBoxesForRange(nominal, isExt), trials(2) As Object
        For i = 0 To 2
            If i < tbs.Count Then
                Dim val As Decimal
                If Decimal.TryParse(tbs(i).Text, val) Then
                    Dim error1 = val, error2 = val - nominal
    ...
    Private Sub CalculateTMU()
        Try
            If _selectedMasters.Count = 0 Then
                TextBox29.Text = "" : TextBox29.Background = New SolidColorBrush(Color.FromRgb(241, 245, 249)) : Return
            End If
            ...
            ' External points
            For Each r In nominalRangesExt
                Dim vals = GetTextBoxesForRange(r, True).Select(Function(t) If(Decimal.TryParse(t.Text, Nothing), Decimal.Parse(t.Text), 0D)).ToArray()
                sumSqMU += (calcStdev(vals) / Math.Sqrt(3)) ^ 2 + (masterU ^ 2)
            Next
            ' Internal points
            For Each r In nominalRangesInt
                Dim vals = GetTextBoxesForRange(r, False).Select(Function(t) If(Decimal.TryParse(t.Text, Nothing), Decimal.Parse(t.Text), 0D)).ToArray()
                sumSqMU += (calcStdev(vals) / Math.Sqrt(3)) ^ 2 + (masterU ^ 2)
            Next
    ...
    Private Sub UpdateDuration(sender As Object, e As TextChangedEventArgs)
        Dim timeInStr = TextBox24.Text.Trim(), timeOutStr = TextBox26.Text.Trim()
        Dim dTimeOut = If(String.IsNullOrEmpty(timeOutStr), DateTime.Now.ToString("HH:mm"), timeOutStr)
        Dim totalMinutes = CalculateDuration(timeInStr, dTimeOut)
