...
Public Class DialGuage100
    Private MySqlCls As New MySQLClass()
    Private _masterLimits As DataRow = Nothing
    Private _instrumentName As String = "DialGauge100" ' Correct name
    ...
    Private ReadOnly observationParameters As String() = {"1_10_Rev", "1_2_Rev", "1_Rev", "Whole_Range", "Hysteresis", "Repeatability"}
    ...
    Private Sub FetchMasterLimits()
        ...
        ' Auto-determine gauge type from the numeric size value
        Dim numericSizeForType As Decimal = 0
        Dim cleanForType = sizeText.Replace("mm", "").Replace(" ", "").Trim()
        If cleanForType.Contains("-") Then
            Dim p = cleanForType.Split("-"c)
            If p.Length > 1 Then Decimal.TryParse(p(1), numericSizeForType)
        Else
            Decimal.TryParse(cleanForType, numericSizeForType)
        End If
        Dim autoGaugeType = If(numericSizeForType <= 50D, "<=50mm", ">50mm")

        If Not String.IsNullOrWhiteSpace(sizeText) AndAlso Not String.IsNullOrWhiteSpace(selectedLC) Then
            Dim mappedSize = MapSizeToMasterRange(sizeText, selectedLC, autoGaugeType)
            _masterLimits = MySqlCls.GetDialGuage100MasterLimits(autoGaugeType, selectedLC, mappedSize)
            TriggerAllValidations()
        End If
    End Sub

    Private Function MapSizeToMasterRange(sizeText As String, lc As String, gaugeType As String) As String
        ...
        If lc = "0.01" Then
            If gaugeType = "<=50mm" Then
                If numericSize <= 1D Then Return "1 or Under"
                If numericSize <= 3D Then Return "Over 1 to and inc 3"
                If numericSize <= 5D Then Return "Over 3 upto and inc 5"
                Return "Over 5 upto and inc 10"
            Else ' >50mm
                If numericSize <= 1D Then Return "1 or Under"
                If numericSize <= 3D Then Return "Over 1 to and inc 3"
                If numericSize <= 5D Then Return "Over 3 upto and inc 5"
                If numericSize <= 10D Then Return "Over 5 upto and inc 10"
                If numericSize <= 20D Then Return "Over 10 upto and inc 20"
                If numericSize <= 30D Then Return "Over 20 upto and inc 30"
                If numericSize <= 50D Then Return "Over 30 upto and inc 50"
                Return "Over 50 upto and inc 100"
            End If
        End If
        ...
        If lc = "0.001" Then
            ...
            Else ' >50mm
                If numericSize <= 1D Then Return "1 or Under"
                If numericSize <= 2D Then Return "Over 1 and upto inc 2"
                Return "Over 2 and upto inc 5"
            End If
        End If
    ...
    Private Sub PopulateCalibrationData()
        ...
        Try
            Dim dt = MySqlCls.GetDialGuage100Data(TextBox1.Text.Trim(), TargetCycle)
            If dt.Rows.Count > 0 Then
                Dim dr = dt.Rows(0)
                _isPopulating = True
                ...
                For Each param In observationParameters
                    For i = 1 To 3
                        Dim tb = DirectCast(Me.FindName($"txtObs_{param}_{i}"), TextBox)
                        If tb IsNot Nothing Then tb.Text = If(IsDBNull(dr($"Obs_{param}_{i}")), "", dr($"Obs_{param}_{i}").ToString())
                    Next
                Next
                ...
                _isPopulating = False
                FetchMasterLimits()  ' Reload master limits with auto gauge type after size/LC are set
                TriggerAllValidations()
                CalculateTMU()
            End If
        Catch ex As Exception
    ...
    Private Sub ObservationKeyDown(sender As Object, e As KeyEventArgs)
        If e.Key = Key.Enter Then
            ...
            If parts.Length >= 4 Then ' txtObs_1_10_Rev_1
    ...
    Private Sub TrialTextChanged(sender As Object, e As TextChangedEventArgs)
        ...
        Try
            ...
            If parts.Length >= 4 AndAlso _masterLimits IsNot Nothing Then
                Dim paramParts = parts.Skip(1).Take(parts.Length - 2)
                Dim param = String.Join("_", paramParts)
                
                Dim ulCol = $"Obs_{param}_UL"
                Dim llCol = $"Obs_{param}_LL"
    ...
    Private Sub Button2_Click(sender As Object, e As RoutedEventArgs)
        ...
        ' Auto-derive gauge type from size (same logic as FetchMasterLimits)
        Dim numSizeForSave As Decimal = 0
        Dim cleanSizeForSave = size.Replace("mm", "").Replace(" ", "").Trim()
        If cleanSizeForSave.Contains("-") Then
            Dim pp = cleanSizeForSave.Split("-"c)
            If pp.Length > 1 Then Decimal.TryParse(pp(1), numSizeForSave)
        Else
            Decimal.TryParse(cleanSizeForSave, numSizeForSave)
        End If
        Dim type = If(numSizeForSave <= 50D, "<=50mm", ">50mm")
        ...
        Dim obs_1_10_Rev = {DirectCast(Me.FindName("txtObs_1_10_Rev_1"), TextBox).Text, DirectCast(Me.FindName("txtObs_1_10_Rev_2"), TextBox).Text, DirectCast(Me.FindName("txtObs_1_10_Rev_3"), TextBox).Text}
        Dim obs_1_2_Rev = {DirectCast(Me.FindName("txtObs_1_2_Rev_1"), TextBox).Text, DirectCast(Me.FindName("txtObs_1_2_Rev_2"), TextBox).Text, DirectCast(Me.FindName("txtObs_1_2_Rev_3"), TextBox).Text}
        Dim obs_1_Rev = {DirectCast(Me.FindName("txtObs_1_Rev_1"), TextBox).Text, DirectCast(Me.FindName("txtObs_1_Rev_2"), TextBox).Text, DirectCast(Me.FindName("txtObs_1_Rev_3"), TextBox).Text}
        Dim obs_Whole_Range = {DirectCast(Me.FindName("txtObs_Whole_Range_1"), TextBox).Text, DirectCast(Me.FindName("txtObs_Whole_Range_2"), TextBox).Text, DirectCast(Me.FindName("txtObs_Whole_Range_3"), TextBox).Text}
        Dim obs_Hysteresis = {DirectCast(Me.FindName("txtObs_Hysteresis_1"), TextBox).Text, DirectCast(Me.FindName("txtObs_Hysteresis_2"), TextBox).Text, DirectCast(Me.FindName("txtObs_Hysteresis_3"), TextBox).Text}
        Dim obs_Repeatability = {DirectCast(Me.FindName("txtObs_Repeatability_1"), TextBox).Text, DirectCast(Me.FindName("txtObs_Repeatability_2"), TextBox).Text, DirectCast(Me.FindName("txtObs_Repeatability_3"), TextBox).Text}

        Dim success As Boolean = False
        If IsEditMode Then
            success = MySqlCls.UpdateDialGuage100(type, controlNo, TargetCycle, size, lc, color, location, temp, humidity, tmu, obs_1_10_Rev, obs_1_2_Rev, obs_1_Rev, obs_Whole_Range, obs_Hysteresis, obs_Repeatability, timeIn, timeOut, totalTime, status, remark, TxtCalibrationDate.SelectedDate)
        Else
            success = MySqlCls.InsertDialGuage100(type, controlNo, cycleName, TxtCalibrationDate.SelectedDate, size, lc, color, location, temp, humidity, tmu, obs_1_10_Rev, obs_1_2_Rev, obs_1_Rev, obs_Whole_Range, obs_Hysteresis, obs_Repeatability, timeIn, timeOut, totalTime, status, remark)
        End If
