...
Public Class ThreadRingGaugeCalibration
    Private MySqlCls As New MySQLClass()
    Private _masterLimits As DataRow = Nothing
    Private _instrumentName As String = "Thread Ring Gauge"
    ...
    Private Sub Window_Loaded(sender As Object, e As RoutedEventArgs) Handles Me.Loaded
        ...
        Try
            ComboLC.Items.Clear()
            Dim dt = MySqlCls.ReadDatatable("SELECT DISTINCT LC FROM thread_ring_gauge_calibration WHERE RowType='MASTER' ORDER BY LC ASC")
        ...
        Dim obsFields = {TxtObs1, TxtObs2, TxtObs3}
        ...
        Dim navTextBoxes = {TxtSize, TxtMin, TxtMax, TxtObs1, TxtObs2, TxtObs3}
        ...
        Try
            ...
            For Each r As DataRow In dtSettings.Rows
                ...
                If dt.Rows.Count > 0 Then
                    ...
                    _instrumentName = If(_category.Equals("Instrument", StringComparison.OrdinalIgnoreCase), If(row.Table.Columns.Contains("InstrumentName"), row("InstrumentName").ToString(), "Thread Ring Gauge"), If(row.Table.Columns.Contains("GaugeName"), row("GaugeName").ToString(), "Thread Ring Gauge"))
    ...
    Private Sub FetchMasterLimits()
        Try
            _masterLimits = MySqlCls.GetThreadRingGaugeMasterLimits(_lc)
    ...
    Private Sub PopulateCalibrationData()
        ...
        _isPopulating = True
        Try
            Dim dt = MySqlCls.GetThreadRingGaugeCalibrationData(TxtControlNo.Text.Trim(), TargetCycle)
            If dt.Rows.Count > 0 Then
                Dim dr = dt.Rows(0)
    ...
    Private Sub UpdateJudgement()
        ...
        If Not String.IsNullOrWhiteSpace(TxtSize.Text) Then
            ...
            If Decimal.TryParse(TxtMin.Text, minVal) AndAlso Decimal.TryParse(TxtMax.Text, maxVal) Then
                Dim obsTbs = {TxtObs1, TxtObs2, TxtObs3}
    ...
    Private Sub BtnUpdate_Click(sender As Object, e As RoutedEventArgs)
        ...
        Dim parameter = "Effective Diameter"
        ...
        If IsEditMode Then
            success = MySqlCls.UpdateThreadRingGaugeCalibration(type, controlNo, TargetCycle, size, lc, color, location, temp, humidity, tmu, mu, parameter, locationObs, minLimit, maxLimit, TxtObs1.Text, TxtObs2.Text, TxtObs3.Text, timeIn, timeOut, totalTime, status, remark, TxtCalibrationDate.SelectedDate)
        Else
            success = MySqlCls.InsertThreadRingGaugeCalibration(type, controlNo, cycleName, TxtCalibrationDate.SelectedDate, size, lc, color, location, temp, humidity, tmu, mu, parameter, locationObs, minLimit, maxLimit, TxtObs1.Text, TxtObs2.Text, TxtObs3.Text, timeIn, timeOut, totalTime, status, remark)
        End If
    ...
    Private Sub CalculateTMU()
        Try
            ...
            If obsVals.Count = 3 Then
                Dim stdevVal = calcStdev(obsVals.ToArray())
                Dim sigB = stdevVal / Math.Sqrt(3)
