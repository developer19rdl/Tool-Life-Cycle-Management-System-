...
Public Class ThreadPitchMicrometer100
    ...
    Private _activeMinRange As Decimal = 75
    Private _activeMaxRange As Decimal = 100

    Private ReadOnly nominalRanges As Decimal() = {77.5, 80.1, 82.7, 85.3, 87.9, 90.0, 92.6, 95.2, 97.8, 100.0}
    ...
    Private Sub Window_Loaded(sender As Object, e As RoutedEventArgs) Handles Me.Loaded
        ...
        If IsEditMode Then PopulateCalibrationData()
    ...
    Private Sub PopulateCalibrationData()
        Try
            Dim dt = MySqlCls.GetThreadPitchMicrometer100Data(TextBox1.Text.Trim(), TargetCycle)
            If dt IsNot Nothing AndAlso dt.Rows.Count > 0 Then
                Dim row = dt.Rows(0)
                If dt.Columns.Contains("LC") Then ComboBox5.Text = row("LC").ToString()
                If dt.Columns.Contains("Size") Then TextBox21.Text = row("Size").ToString()

                Dim safeAssign = Sub(tb As TextBox, col As String)
                                     If dt.Columns.Contains(col) AndAlso Not IsDBNull(row(col)) Then
                                         Dim val = row(col).ToString()
                                         If val <> "-" Then tb.Text = val
                                     End If
                                 End Sub

                Dim obsCols = {"Obs77_5", "Obs80_1", "Obs82_7", "Obs85_3", "Obs87_9", "Obs90_0", "Obs92_6", "Obs95_2", "Obs97_8", "Obs100_0"}
                For i = 0 To nominalRanges.Length - 1
                    Dim tbs = GetTextBoxesForRange(nominalRanges(i))
                    Dim colPrefix = obsCols(i)
                    For j = 0 To 2
                        If tbs.Count > j Then safeAssign(tbs(j), $"{colPrefix}_{j + 1}")
                    Next
                Next

                safeAssign(TextBox30, "Temperature")
                safeAssign(TextBox28, "Humidity")
                safeAssign(TextBox20, "DepthError")

                If dt.Columns.Contains("Date") AndAlso Not IsDBNull(row("Date")) Then
                    Dim d As DateTime
                    If DateTime.TryParse(row("Date").ToString(), d) Then TxtCalibrationDate.SelectedDate = d
                End If

                safeAssign(TextBox24, "TimeIn")
                safeAssign(TextBox26, "TimeOut")
                safeAssign(TextBox25, "TotalTime")
                safeAssign(TextBox19, "Remark")
                ...
                If dt.Columns.Contains("TMU") AndAlso Not IsDBNull(row("TMU")) Then
                    TextBox29.Text = row("TMU").ToString()
                    TextBox29.Background = Brushes.White
                End If
            End If
        Catch ex As Exception
            MessageBox.Show("Error loading calibration data: " & ex.Message, "Error", MessageBoxButton.OK, MessageBoxImage.Error)
        End Try
    End Sub
    ...
    Private Sub AdjustVisibilityBySize(sizeStr As String)
        Try
            _activeMinRange = 75 : _activeMaxRange = 100
    ...
    Private Sub TrialTextChanged(sender As Object, e As TextChangedEventArgs)
        ...
        ' Special case for 100.0 to match the database column naming
        If nominal = 100D Then nomStr = "100_0"
    ...
    Private Function GetTextBoxesForRange(nominal As Decimal) As List(Of TextBox)
        ...
        Dim nomStr = nominal.ToString("0.0").Replace(".", "_")
        ' Special case for 100.0 to match the xaml naming
        If nominal = 100D Then nomStr = "100_0"
    ...
    Private Sub LoadInstrumentDetails(controlNo As String)
        Try
            ...
            Next
            If Not found Then
                _instrumentName = "" : ComboBox2.Text = "" : TextBoxColor.Text = "" : TextBox21.Text = "75-100 mm" : TextBox21.SelectedIndex = -1
            End If
        Catch ex As Exception
        End Try
    End Sub

    Private Function TryMatchSize(dbSize As String, comboItems As List(Of String)) As String
        If String.IsNullOrWhiteSpace(dbSize) Then Return ""
        
        ' Normalize dbSize (e.g., "100" or "0-100 mm")
        Dim normalizedDb = dbSize.Replace("mm", "").Replace("MM", "").Trim()
        
        ' Try exact match first
        For Each item In comboItems
            If item.Equals(dbSize, StringComparison.OrdinalIgnoreCase) Then Return item
        Next
        
        ' Try range endpoint match (e.g., "100" matches "0-100 mm")
        For Each item In comboItems
            Dim itemNorm = item.Replace("mm", "").Replace("MM", "").Trim()
            if itemNorm.Contains("-") Then
                Dim parts = itemNorm.Split("-"c)
                If parts.Length > 1 Then
                    Dim endRange = parts(1).Trim()
                    If endRange = normalizedDb Then Return item
                End If
            Else
                If itemNorm = normalizedDb Then Return item
            End If
        Next
        
        ' Default to empty if no good match
        Return ""
    End Function
    ...
    Private Sub LoadDynamicLC()
        Try
            Dim dt = MySqlCls.ReadDatatable("SELECT DISTINCT LC FROM thread_pitch_micrometer_100 WHERE RowType='MASTER' ORDER BY LC ASC")
    ...
    Private Sub ComboBox5_SelectionChanged(sender As Object, e As SelectionChangedEventArgs) Handles ComboBox5.SelectionChanged
        If ComboBox5.SelectedItem IsNot Nothing Then
            ...
            _masterLimits = MySqlCls.GetThreadPitchMicrometer100MasterLimits(selectedLC)
            ResetAllTrials() : EnableRange(nominalRanges(0))
        Else
    ...
    Private Sub Button2_Click(sender As Object, e As RoutedEventArgs) Handles Button2.Click
        ...
        Dim obs77_5 = GetTrialsArrayFromUI(nominalRanges(0))
        Dim obs80_1 = GetTrialsArrayFromUI(nominalRanges(1))
        Dim obs82_7 = GetTrialsArrayFromUI(nominalRanges(2))
        Dim obs85_3 = GetTrialsArrayFromUI(nominalRanges(3))
        Dim obs87_9 = GetTrialsArrayFromUI(nominalRanges(4))
        Dim obs90_0 = GetTrialsArrayFromUI(nominalRanges(5))
        Dim obs92_6 = GetTrialsArrayFromUI(nominalRanges(6))
        Dim obs95_2 = GetTrialsArrayFromUI(nominalRanges(7))
        Dim obs97_8 = GetTrialsArrayFromUI(nominalRanges(8))
        Dim obs100_0 = GetTrialsArrayFromUI(nominalRanges(9))

        Dim saved As Boolean
        If IsEditMode Then
            saved = MySqlCls.UpdateThreadPitchMicrometer100(
                _instrumentName, TextBox1.Text.Trim(), TargetCycle, TextBox21.Text, ComboBox5.Text, TextBoxColor.Text, ComboBox2.Text, TextBox30.Text, TextBox28.Text, TextBox29.Text,
                obs77_5, obs80_1, obs82_7, obs85_3, obs87_9, obs90_0, obs92_6, obs95_2, obs97_8, obs100_0,
                ...
                calibDateToSave)
        Else
            saved = MySqlCls.InsertThreadPitchMicrometer100(
                _instrumentName, TextBox1.Text.Trim(), cycleName, TextBox21.Text, ComboBox5.Text, TextBoxColor.Text, ComboBox2.Text, TextBox30.Text, TextBox28.Text, TextBox29.Text,
                obs77_5, obs80_1, obs82_7, obs85_3, obs87_9, obs90_0, obs92_6, obs95_2, obs97_8, obs100_0,
                If(String.IsNullOrEmpty(TextBox20.Text), "-", TextBox20.Text), TextBox24.Text, TextBox26.Text, TextBox25.Text, statusVal, TextBox19.Text,
                calibDateToSave)
        End If
        ...
        For i = 0 To 2
            If i < tbs.Count Then
                ...
                If Decimal.TryParse(tbs(i).Text, val) Then
                    ...
                    trials(i) = errorVal.ToString("0.000")
                Else
                    trials(i) = "-"
                End If
            Else
