...
Public Class PlainRingGaugeCalibration
    Private MySqlCls As New MySQLClass()
    Private _masterLimits As DataRow = Nothing
    Private _instrumentName As String = "Plain Ring Gauge"
    ...
    Private Sub Window_Loaded(sender As Object, e As RoutedEventArgs) Handles Me.Loaded
        ...
        Try
            ComboLC.Items.Clear()
            Dim dt = MySqlCls.ReadDatatable("SELECT DISTINCT LC FROM plain_ring_gauge_calibration WHERE RowType='MASTER' ORDER BY LC ASC")
        ...
        Dim trialBoxes = {TxtObs1, TxtObs2, TxtObs3, TxtObs4}
        ...
        AddHandler TxtMin.TextChanged, Sub()
                                           TrialTextChanged(TxtObs1, Nothing)
                                           TrialTextChanged(TxtObs2, Nothing)
                                           TrialTextChanged(TxtObs3, Nothing)
                                           TrialTextChanged(TxtObs4, Nothing)
                                       End Sub
        AddHandler TxtMax.TextChanged, Sub()
                                           TrialTextChanged(TxtObs1, Nothing)
                                           TrialTextChanged(TxtObs2, Nothing)
                                           TrialTextChanged(TxtObs3, Nothing)
                                           TrialTextChanged(TxtObs4, Nothing)
        ...
        Dim navTextBoxes = {TxtSize, TxtMin, TxtMax, TxtObs1, TxtObs2, TxtObs3, TxtObs4}
        ...
        Try
            ...
            For Each r As DataRow In dtSettings.Rows
                ...
                If dt.Rows.Count > 0 Then
                    ...
                    _instrumentName = If(_category.Equals("Instrument", StringComparison.OrdinalIgnoreCase), If(row.Table.Columns.Contains("InstrumentName"), row("InstrumentName").ToString(), "Plain Ring Gauge"), If(row.Table.Columns.Contains("GaugeName"), row("GaugeName").ToString(), "Plain Ring Gauge"))
    ...
    Private Sub FetchMasterLimits()
        Try
            _masterLimits = MySqlCls.GetPlainRingGaugeMasterLimits(_lc)
            If _masterLimits IsNot Nothing AndAlso IsEditMode Then
                ' Populate limits UI if controls exist
                If _masterLimits.Table.Columns.Contains("Min_Limit") AndAlso Not IsDBNull(_masterLimits("Min_Limit")) Then
                    TxtMin.Text = _masterLimits("Min_Limit").ToString()
                End If
                If _masterLimits.Table.Columns.Contains("Max_Limit") AndAlso Not IsDBNull(_masterLimits("Max_Limit")) Then
                    TxtMax.Text = _masterLimits("Max_Limit").ToString()
                End If
            ElseIf Not IsEditMode Then
                If _masterLimits IsNot Nothing Then
                    If _masterLimits.Table.Columns.Contains("Min_Limit") AndAlso Not IsDBNull(_masterLimits("Min_Limit")) Then TxtMin.Text = _masterLimits("Min_Limit").ToString()
                    If _masterLimits.Table.Columns.Contains("Max_Limit") AndAlso Not IsDBNull(_masterLimits("Max_Limit")) Then TxtMax.Text = _masterLimits("Max_Limit").ToString()
                Else
                    TxtMin.Text = ""
                    TxtMax.Text = ""
                End If
    ...
    Private Sub PopulateCalibrationData()
        ...
        _isPopulating = True
        Try
            Dim dt = MySqlCls.GetPlainRingGaugeCalibrationData(TxtControlNo.Text.Trim(), TargetCycle)
            If dt.Rows.Count > 0 Then
                Dim dr = dt.Rows(0)
                ...
                End If
                If Not IsDBNull(dr("Min_Limit")) Then TxtMin.Text = dr("Min_Limit").ToString()
                If Not IsDBNull(dr("Max_Limit")) Then TxtMax.Text = dr("Max_Limit").ToString()
                If Not IsDBNull(dr("Obs_1")) Then TxtObs1.Text = dr("Obs_1").ToString()
                If Not IsDBNull(dr("Obs_2")) Then TxtObs2.Text = dr("Obs_2").ToString()
                If Not IsDBNull(dr("Obs_3")) Then TxtObs3.Text = dr("Obs_3").ToString()
                If Not IsDBNull(dr("Obs_4")) Then TxtObs4.Text = dr("Obs_4").ToString()
    ...
    Private Sub TrialTextChanged(sender As Object, e As TextChangedEventArgs)
        ...
        If Decimal.TryParse(tb.Text, val) Then
            ...
            ll = If(Decimal.TryParse(TxtMin.Text, minVal), minVal, -999999.0)
            ul = If(Decimal.TryParse(TxtMax.Text, maxVal), maxVal, 999999.0)
    ...
    Private Sub UpdateJudgement()
        ...
        For Each tb In trialBoxes
            If tb IsNot Nothing AndAlso Not String.IsNullOrWhiteSpace(tb.Text) Then
                ...
                If Decimal.TryParse(tb.Text, val) Then
                    Dim minVal, maxVal As Decimal
                    Dim ll = If(Decimal.TryParse(TxtMin.Text, minVal), minVal, -999999.0)
                    Dim ul = If(Decimal.TryParse(TxtMax.Text, maxVal), maxVal, 999999.0)
                    If val < ll OrElse val > ul Then
                        isAnyNG = True
                    End If
                Else
    ...
    Private Sub BtnUpdate_Click(sender As Object, e As RoutedEventArgs)
        ...
        Dim parameter = "Internal Diameter"
        Dim locationObs = ""
        ...
        Dim tempMin, tempMax As Decimal
        Dim min_lim As Object = If(Decimal.TryParse(TxtMin.Text, tempMin), tempMin, DBNull.Value)
        Dim max_lim As Object = If(Decimal.TryParse(TxtMax.Text, tempMax), tempMax, DBNull.Value)
        Dim obsDetails = {TxtObs1.Text, TxtObs2.Text, TxtObs3.Text, TxtObs4.Text}

        Dim success As Boolean = False
        Dim targetCyc = If(IsEditMode, TargetCycle, cycleName)

        If IsEditMode Then
            success = MySqlCls.UpdatePlainRingGaugeCalibration(type, controlNo, TargetCycle, size, lc, color, location, temp, humidity, tmu, mu, parameter, locationObs, min_lim, max_lim, obsDetails, timeIn, timeOut, totalTime, status, remark, TxtCalibrationDate.SelectedDate)
        Else
            success = MySqlCls.InsertPlainRingGaugeCalibration(type, controlNo, cycleName, TxtCalibrationDate.SelectedDate, size, lc, color, location, temp, humidity, tmu, mu, parameter, locationObs, min_lim, max_lim, obsDetails, timeIn, timeOut, totalTime, status, remark)
        End If
    ...
    Private Sub CalculateTMU()
        Try
            ...
            ' 2. Calculate stdev for observations
            Dim obsVals As New System.Collections.Generic.List(Of Decimal)()
            Dim obsTbs = {TxtObs1, TxtObs2, TxtObs3, TxtObs4}
            For Each tb In obsTbs
                Dim val As Decimal
                If Decimal.TryParse(tb.Text, val) Then
                    obsVals.Add(val)
                End If
            ...
            ' Only calculate if we have all 4 values
            If obsVals.Count = 4 Then
                Dim stdev = calcStdev(obsVals.ToArray())
                Dim sigB1 = stdev / Math.Sqrt(4)
                Dim mu_sq = (sigB1 ^ 2) + (masterU ^ 2)

                ' Final TMU
                Dim preTMU = Math.Sqrt(mu_sq)
