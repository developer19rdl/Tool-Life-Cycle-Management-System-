...
Public Class RadiusGaugeCalibration
    Private MySqlCls As New MySQLClass()
    Private _masterLimits As DataRow = Nothing
    Private _instrumentName As String = "Radius Gauge"
    ...
    Private _observationRows As New List(Of RadiusObservationRow)()
    Private _navTextBoxes As New List(Of TextBox)()


    Public Class RadiusObservationRow
        Public SizeBox As TextBox
        Public MinBox As TextBox
        Public MaxBox As TextBox
        Public InsideBoxes As TextBox()
        Public OutsideBoxes As TextBox()
    ...
    Private Sub Window_Loaded(sender As Object, e As RoutedEventArgs) Handles Me.Loaded
        ...
        Try
            ComboLC.Items.Clear()
            Dim dt = MySqlCls.ReadDatatable("SELECT DISTINCT LC FROM radius_gauge_calibration WHERE RowType='MASTER' ORDER BY LC ASC")
        ...
        Try
            ...
            For Each r As DataRow In dtSettings.Rows
                ...
                If dt.Rows.Count > 0 Then
                    ...
                    _instrumentName = If(_category.Equals("Instrument", StringComparison.OrdinalIgnoreCase), If(row.Table.Columns.Contains("InstrumentName"), row("InstrumentName").ToString(), "Radius Gauge"), If(row.Table.Columns.Contains("GaugeName"), row("GaugeName").ToString(), "Radius Gauge"))
    ...
    Private Sub FetchMasterLimits()
        Try
            _masterLimits = MySqlCls.GetRadiusGaugeMasterLimits(_lc)
    ...
    Private Sub PopulateCalibrationData()
        ...
        _isPopulating = True
        Try
            Dim masterRow = MySqlCls.GetRadiusGaugeMasterData(TxtControlNo.Text.Trim(), TargetCycle)
            If masterRow IsNot Nothing Then
                ...
                Dim dtRecords = MySqlCls.GetRadiusGaugeRecordData(TxtControlNo.Text.Trim(), TargetCycle)
                ...
                For idx As Integer = 0 To dtRecords.Rows.Count - 1
                    ...
                    If Not IsDBNull(recordRow("Inside_Radius_Obs_1")) Then rowObj.InsideBoxes(0).Text = recordRow("Inside_Radius_Obs_1").ToString()
                    If Not IsDBNull(recordRow("Inside_Radius_Obs_2")) Then rowObj.InsideBoxes(1).Text = recordRow("Inside_Radius_Obs_2").ToString()
                    If Not IsDBNull(recordRow("Inside_Radius_Obs_3")) Then rowObj.InsideBoxes(2).Text = recordRow("Inside_Radius_Obs_3").ToString()

                    If Not IsDBNull(recordRow("Outside_Radius_Obs_1")) Then rowObj.OutsideBoxes(0).Text = recordRow("Outside_Radius_Obs_1").ToString()
                    If Not IsDBNull(recordRow("Outside_Radius_Obs_2")) Then rowObj.OutsideBoxes(1).Text = recordRow("Outside_Radius_Obs_2").ToString()
                    If Not IsDBNull(recordRow("Outside_Radius_Obs_3")) Then rowObj.OutsideBoxes(2).Text = recordRow("Outside_Radius_Obs_3").ToString()
                Next
    ...
    Private Sub ValidateRowBoxes(row As RadiusObservationRow)
        If _isPopulating Then Return
        For Each tb In row.InsideBoxes
            ValidateBox(tb, row.MinBox.Text, row.MaxBox.Text)
        Next
        For Each tb In row.OutsideBoxes
            ValidateBox(tb, row.MinBox.Text, row.MaxBox.Text)
        Next
    End Sub
    ...
    Private Sub GenerateObservationRows(numRows As Integer)
        ...
        For i As Integer = 1 To numRows
            ...
            ' Create Inside Obs 1, 2, 3
            Dim insideBoxes(2) As TextBox
            For j As Integer = 0 To 2
                insideBoxes(j) = New TextBox() With {.HorizontalAlignment = HorizontalAlignment.Stretch, .Margin = New Thickness(2)}
                Grid.SetColumn(insideBoxes(j), 3 + j)
                rowGrid.Children.Add(insideBoxes(j))
            Next

            ' Create Outside Obs 1, 2, 3
            Dim outsideBoxes(2) As TextBox
            For j As Integer = 0 To 2
                outsideBoxes(j) = New TextBox() With {.HorizontalAlignment = HorizontalAlignment.Stretch, .Margin = New Thickness(2)}
                Grid.SetColumn(outsideBoxes(j), 6 + j)
                rowGrid.Children.Add(outsideBoxes(j))
            Next

            Dim rowObj As New RadiusObservationRow() With {
                .SizeBox = txtSize,
                .MinBox = txtMin,
                .MaxBox = txtMax,
                .InsideBoxes = insideBoxes,
                .OutsideBoxes = outsideBoxes,
                .RowGrid = rowGrid
            }
            ...
            For j As Integer = 0 To 2
                AddHandler insideBoxes(j).TextChanged, Sub(sender, args)
                                                          ValidateBox(TryCast(sender, TextBox), txtMin.Text, txtMax.Text)
                                                      End Sub
                AddHandler outsideBoxes(j).TextChanged, Sub(sender, args)
                                                           ValidateBox(TryCast(sender, TextBox), txtMin.Text, txtMax.Text)
                                                       End Sub
            Next
            ...
            For Each tb In insideBoxes
                _navTextBoxes.Add(tb)
            Next
            For Each tb In outsideBoxes
                _navTextBoxes.Add(tb)
            Next
    ...
    Private Sub BtnUpdate_Click(sender As Object, e As RoutedEventArgs)
        ...
        MySqlCls.DeleteRadiusGaugeCalibration(controlNo, targetCyc)

        ' Insert MASTER row
        Dim success = MySqlCls.InsertRadiusGaugeMaster(controlNo, targetCyc, calibDate, type, size, lc, color, location, temp, humidity, tmu, mu, tempMin, tempMax, humMin, humMax, status, remark, ProjectSettings.Current.Username, timeIn, timeOut, totalTime)
        ...
        If success Then
            ...
            For Each row In _observationRows
                ...
                Dim i1 As Decimal? = Nothing
                Dim i2 As Decimal? = Nothing
                Dim i3 As Decimal? = Nothing
                If Decimal.TryParse(row.InsideBoxes(0).Text, decVal) Then i1 = decVal
                If Decimal.TryParse(row.InsideBoxes(1).Text, decVal) Then i2 = decVal
                If Decimal.TryParse(row.InsideBoxes(2).Text, decVal) Then i3 = decVal

                Dim o1 As Decimal? = Nothing
                Dim o2 As Decimal? = Nothing
                Dim o3 As Decimal? = Nothing
                If Decimal.TryParse(row.OutsideBoxes(0).Text, decVal) Then o1 = decVal
                If Decimal.TryParse(row.OutsideBoxes(1).Text, decVal) Then o2 = decVal
                If Decimal.TryParse(row.OutsideBoxes(2).Text, decVal) Then o3 = decVal
                ...
                MySqlCls.InsertRadiusGaugeRecord(controlNo, targetCyc, calibDate, type, size, lc, color, location, measRange, minLimit, maxLimit, i1, i2, i3, o1, o2, o3, rowJudgement, "", ProjectSettings.Current.Username)
            Next
    ...
    Private Sub CalculateTMU()
        Try
            ...
            ' Row-by-Row TMU calculation based on Inside and Outside Readings
            Dim sumSqMU As Decimal = 0
            Dim hasValidRow As Boolean = False

            For Each row In _observationRows
                ' Inside Radius
                Dim insideVals As New List(Of Decimal)()
                For Each tb In row.InsideBoxes
                    Dim val As Decimal
                    If Decimal.TryParse(tb.Text, val) Then insideVals.Add(val)
                Next

                If insideVals.Count > 0 Then
                    ' 1. Calculate Standard Deviation in Microns (for precision)
                    Dim microns = insideVals.Select(Function(v) v * 1000D).ToArray()
                    Dim stdev_microns As Decimal = If(insideVals.Count > 1, calcStdev(microns), 0)
                    
                    ' 2. Convert back to mm and calculate Sigma B1
                    Dim stdev_mm As Decimal = stdev_microns / 1000D
                    Dim sigB1_row As Decimal = stdev_mm / Math.Sqrt(insideVals.Count)
                ...
                ' Outside Radius
                Dim outsideVals As New List(Of Decimal)()
                For Each tb In row.OutsideBoxes
                    Dim val As Decimal
                    If Decimal.TryParse(tb.Text, val) Then outsideVals.Add(val)
                Next

                If outsideVals.Count > 0 Then
                    ' 1. Calculate Standard Deviation in Microns (for precision)
                    Dim microns = outsideVals.Select(Function(v) v * 1000D).ToArray()
                    Dim stdev_microns As Decimal = If(outsideVals.Count > 1, calcStdev(microns), 0)
                    
                    ' 2. Convert back to mm and calculate Sigma B1
                    Dim stdev_mm As Decimal = stdev_microns / 1000D
                    Dim sigB1_row As Decimal = stdev_mm / Math.Sqrt(outsideVals.Count)
