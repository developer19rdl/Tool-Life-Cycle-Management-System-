...
Public Class GeartoothMicrometer50
    ...
    Private ReadOnly nominalRanges As Decimal() = {0, 5, 10, 15, 20, 25, 30, 35, 40, 45, 50}
    ...
    Private Sub Window_Loaded(sender As Object, e As RoutedEventArgs) Handles Me.Loaded
        ComboBox1.Text = "Gear Tooth Micrometer"
        LoadDynamicLC()
    ...
    Private Sub PopulateCalibrationData()
        Try
            Dim dt = MySqlCls.GetGeartoothMicrometer50Data(TextBox1.Text.Trim(), TargetCycle)
            If dt.Rows.Count > 0 Then
                ...
                ' Populate Trials
                For Each r In nominalRanges
                    SetTrialsFromDB(row, $"Obs{r}_0_", GetTextBoxesForRange(r))
                Next
    ...
    Private Sub SetTrialsFromDB(row As DataRow, prefix As String, tbs As List(Of TextBox))
        For i = 0 To 2
            If i < tbs.Count Then
                ' Geartooth columns are named like Obs0_0_1, Obs0_0_2, Obs0_0_3
    ...
    Private Sub AdjustVisibilityBySize(sizeStr As String)
        Try
            _activeMinRange = 0 : _activeMaxRange = 50
    ...
    Private Sub TrialTextChanged(sender As Object, e As TextChangedEventArgs)
        ...
        ' Using ObsX_0_UL/LL as per database schema
        Dim prefix As String = "Obs"
        Dim ulCol As String = $"{prefix}{nominal}_0_UL"
        Dim llCol As String = $"{prefix}{nominal}_0_LL"
    ...
    Private Sub LoadInstrumentDetails(controlNo As String)
        Try
            ...
            Next
            If Not found Then
                _instrumentName = "" : ComboBox2.Text = "" : TextBoxColor.Text = "" : TextBox21.Text = "0-50 mm" : UpdateSizeLockState()
            End If
        Catch ex As Exception
        End try
    End Sub
    ...
    Private Sub LoadDynamicLC()
        Try
            Dim dt = MySqlCls.ReadDatatable("SELECT DISTINCT LC FROM geartooth_micrometer_50 WHERE RowType='MASTER' ORDER BY LC ASC")
    ...
    Private Sub ComboBox5_SelectionChanged(sender As Object, e As SelectionChangedEventArgs) Handles ComboBox5.SelectionChanged
        If ComboBox5.SelectedItem IsNot Nothing Then
            ...
            _masterLimits = MySqlCls.GetGeartoothMicrometer50MasterLimits(selectedLC)
            ResetAllTrials()
            AdjustVisibilityBySize(TextBox21.Text)
            EnableRange(0)
        Else
    ...
    Private Sub Button2_Click(sender As Object, e As RoutedEventArgs) Handles Button2.Click
        ...
        Dim obs0 = GetTrialsArrayFromUI(0), obs5 = GetTrialsArrayFromUI(5), obs10 = GetTrialsArrayFromUI(10)
        Dim obs15 = GetTrialsArrayFromUI(15), obs20 = GetTrialsArrayFromUI(20), obs25 = GetTrialsArrayFromUI(25)
        Dim obs30 = GetTrialsArrayFromUI(30), obs35 = GetTrialsArrayFromUI(35), obs40 = GetTrialsArrayFromUI(40)
        Dim obs45 = GetTrialsArrayFromUI(45), obs50 = GetTrialsArrayFromUI(50)

        Dim saved As Boolean = False
        If IsEditMode Then
            saved = MySqlCls.UpdateGeartoothMicrometer50(
                ComboBox1.Text, TextBox1.Text.Trim(), TargetCycle, TextBox21.Text, ComboBox5.Text, TextBoxColor.Text, ComboBox2.Text, TextBox30.Text, TextBox28.Text, TextBox29.Text,
                obs0, obs5, obs10, obs15, obs20, obs25, obs30, obs35, obs40, obs45, obs50,
                ...
                calibDateToSave)
        Else
            saved = MySqlCls.InsertGeartoothMicrometer50(
                ComboBox1.Text, TextBox1.Text.Trim(), cycleName, TextBox21.Text, ComboBox5.Text, TextBoxColor.Text, ComboBox2.Text, TextBox30.Text, TextBox28.Text, TextBox29.Text,
                obs0, obs5, obs10, obs15, obs20, obs25, obs30, obs35, obs40, obs45, obs50,
                If(String.IsNullOrEmpty(TextBox20.Text), "-", TextBox20.Text), TextBox24.Text, TextBox26.Text, TextBox25.Text, statusVal, TextBox19.Text,
                calibDateToSave)
        End If
        ...
        If saved Then
            ...
            If statusVal = "NG" Then
                ...
                Dim dtDept = MySqlCls.ReadDatatable($"SELECT * FROM department_list WHERE `Control No` = '{ctrlNo.Replace("'", "''")}' AND CycleName = '{targetCyc.Replace("'", "''")}' LIMIT 1")
                If dtDept.Rows.Count > 0 Then
                    ...
                    MySqlCls.InsertNGRecord(ctrlNo, instName, dept, targetCyc, ngReason, calibDateToSave, dueDate, statusVal)

                    MySqlCls.InsertWOPRecord(ctrlNo, targetCyc, dept, instName, color, wopReason)
                    MySqlCls.InsertInterchangeRecord(targetCyc, ctrlNo, dept, instName, size, color, "WOP", wopReason)
