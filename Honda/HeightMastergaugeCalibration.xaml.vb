...
Public Class HeightMastergaugeCalibration
    Private MySqlCls As New MySQLClass()
    Private _masterLimits As DataRow = Nothing
    Private _instrumentName As String = "Height Master Gauge"
    ...
    Private Sub Window_Loaded(sender As Object, e As RoutedEventArgs) Handles Me.Loaded
        ...
        Try
            ComboLC.Items.Clear()
            Dim dt = MySqlCls.ReadDatatable("SELECT DISTINCT LC FROM height_master_calibration WHERE RowType='MASTER' ORDER BY LC ASC")
        ...
        AddHandler TxtMU.TextChanged, AddressOf TxtMU_TextChanged

        ' Observations TextChanged Handlers
        Dim trialBoxes = {TxtGo1, TxtGo2, TxtGo3, TxtNoGo1, TxtNoGo2, TxtNoGo3}
        ...
        Dim navTextBoxes = {TxtGoSize, TxtGoMin, TxtGoMax, TxtGo1, TxtGo2, TxtGo3,
                            TxtNoGoSize, TxtNoGoMin, TxtNoGoMax, TxtNoGo1, TxtNoGo2, TxtNoGo3}
        ...
        Try
            ...
            For Each r As DataRow In dtSettings.Rows
                ...
                If dt.Rows.Count > 0 Then
                    ...
                    If Not String.IsNullOrEmpty(matchedSize) AndAlso IsEditMode Then
                    ...
                    _instrumentName = If(_category.Equals("Instrument", StringComparison.OrdinalIgnoreCase), If(row.Table.Columns.Contains("InstrumentName"), row("InstrumentName").ToString(), "Height Master Gauge"), If(row.Table.Columns.Contains("GaugeName"), row("GaugeName").ToString(), "Height Master Gauge"))
    ...
    Private Sub FetchMasterLimits()
        Try
            _masterLimits = MySqlCls.GetHeightMasterGaugeMasterLimits(_lc)
    ...
    Private Sub PopulateCalibrationData()
        ...
        _isPopulating = True
        Try
            Dim dt = MySqlCls.GetHeightMasterGaugeCalibrationData(TxtControlNo.Text.Trim(), TargetCycle)
            If dt.Rows.Count > 0 Then
                Dim dr = dt.Rows(0)
                ...
                If Not IsDBNull(dr("Go_Size")) Then TxtGoSize.Text = dr("Go_Size").ToString()
                ...
                If Not IsDBNull(dr("NoGo_Size")) Then TxtNoGoSize.Text = dr("NoGo_Size").ToString()
                ...
                If Not IsDBNull(dr("Judgement")) Then
                    ComboJudgement.Text = dr("Judgement").ToString()
                End If
    ...
    Private Sub EnvironmentTextChanged(sender As Object, e As TextChangedEventArgs)
        ...
        If Decimal.TryParse(tb.Text, val) Then
            ...
            If tb.Name = "TxtTemp" Then
                Dim ll = If(_masterLimits IsNot Nothing AndAlso _masterLimits.Table.Columns.Contains("Env_Temp_LL"), Convert.ToDecimal(_masterLimits("Env_Temp_LL")), 18.0)
                Dim ul = If(_masterLimits IsNot Nothing AndAlso _masterLimits.Table.Columns.Contains("Env_Temp_UL"), Convert.ToDecimal(_masterLimits("Env_Temp_UL")), 22.0)
                If val < ll OrElse val > ul Then isOK = False
            ElseIf tb.Name = "TxtHumidity" Then
                Dim ll = If(_masterLimits IsNot Nothing AndAlso _masterLimits.Table.Columns.Contains("Env_Hum_LL"), Convert.ToDecimal(_masterLimits("Env_Hum_LL")), 40.0)
                Dim ul = If(_masterLimits IsNot Nothing AndAlso _masterLimits.Table.Columns.Contains("Env_Hum_UL"), Convert.ToDecimal(_masterLimits("Env_Hum_UL")), 60.0)
                If val < ll OrElse val > ul Then isOK = False
            End If
    ...
    Private Sub BtnUpdate_Click(sender As Object, e As RoutedEventArgs)
        ...
        ' Limits Objects
        Dim flatness_ul = If(_masterLimits IsNot Nothing AndAlso _masterLimits.Table.Columns.Contains("Flatness_UL"), _masterLimits("Flatness_UL"), 1.0)
        Dim flatness_ll = If(_masterLimits IsNot Nothing AndAlso _masterLimits.Table.Columns.Contains("Flatness_LL"), _masterLimits("Flatness_LL"), 0.0)
        Dim flatnessLimits = {flatness_ul, flatness_ll}

        Dim parallelism_ul = If(_masterLimits IsNot Nothing AndAlso _masterLimits.Table.Columns.Contains("Parallelism_UL"), _masterLimits("Parallelism_UL"), 1.0)
        Dim parallelism_ll = If(_masterLimits IsNot Nothing AndAlso _masterLimits.Table.Columns.Contains("Parallelism_LL"), _masterLimits("Parallelism_LL"), 0.0)
        Dim parallelismLimits = {parallelism_ul, parallelism_ll}
        ...
        Dim goDetails = {TxtGoSize.Text, go_min, go_max, TxtGo1.Text, TxtGo2.Text, TxtGo3.Text}
        ...
        Dim noGoDetails = {TxtNoGoSize.Text, nogo_min, nogo_max, TxtNoGo1.Text, TxtNoGo2.Text, TxtNoGo3.Text}

        Dim success As Boolean = False
        Dim targetCyc = If(IsEditMode, TargetCycle, cycleName)

        If IsEditMode Then
            success = MySqlCls.UpdateHeightMasterGaugeCalibration(type, controlNo, TargetCycle, size, lc, color, location, temp, humidity, tmu, mu, flatnessLimits, parallelismLimits, goDetails, noGoDetails, timeIn, timeOut, totalTime, status, remark, TxtCalibrationDate.SelectedDate)
        Else
            success = MySqlCls.InsertHeightMasterGaugeCalibration(type, controlNo, cycleName, TxtCalibrationDate.SelectedDate, size, lc, color, location, temp, humidity, tmu, mu, flatnessLimits, parallelismLimits, goDetails, noGoDetails, timeIn, timeOut, totalTime, status, remark)
        End If
    ...
    Private Sub TxtMU_TextChanged(sender As Object, e As TextChangedEventArgs)
        CalculateTMU()
    End Sub

    Private Sub CalculateTMU()
        Try
            ' Determine MU: prefer manually entered TxtMU, else derive from selected masters
            Dim masterU As Decimal = 0
            Dim muEntered As Decimal
            If Decimal.TryParse(TxtMU.Text, muEntered) AndAlso muEntered > 0 Then
                ' Use manually entered MU value directly
                masterU = muEntered
            ElseIf _selectedMasters IsNot Nothing AndAlso _selectedMasters.Count > 0 Then
                ' Derive from RSS of selected master uncertainties
            ...
            ' Calculate stdev for GO observations
            Dim goVals As New System.Collections.Generic.List(Of Decimal)()
            Dim goTbs = {TxtGo1, TxtGo2, TxtGo3}
            ...
            Dim nogoTbs = {TxtNoGo1, TxtNoGo2, TxtNoGo3}
            ...
            ' Only calculate if we have all 3 values for GO and NOGO
            If goVals.Count = 3 AndAlso nogoVals.Count = 3 Then
                Dim stdev_go = calcStdev(goVals.ToArray())
                Dim sigB1_go = stdev_go / Math.Sqrt(3)
                Dim mu_go_sq = (sigB1_go ^ 2) + (masterU ^ 2)

                Dim stdev_nogo = calcStdev(nogoVals.ToArray())
                Dim sigB1_nogo = stdev_nogo / Math.Sqrt(3)
