...
Public Class ThreadPitchMicrometer50
    ...
    Private Sub PopulateCalibrationData()
        Try
            Dim dt = MySqlCls.GetThreadPitchMicrometer50Data(TextBox1.Text.Trim(), TargetCycle)
            If dt IsNot Nothing AndAlso dt.Rows.Count > 0 Then
                ...
                Dim obsCols = {"Obs27_5", "Obs30_1", "Obs32_7", "Obs35_3", "Obs37_9", "Obs40_0", "Obs42_6", "Obs45_2", "Obs47_8", "Obs50_0"}
    ...
    Private Sub TrialTextChanged(sender As Object, e As TextChangedEventArgs)
        ...
        Try
            ...
            If Decimal.TryParse(txtBox.Text, val) Then
                ' Logic: error is (Actual - Nominal). If point micrometer handles absolute values, 
                ' check which error makes more sense contextually.
                ' For micrometer calibration normally we compare Actual vs Nominal.
                Dim error1 As Decimal = val
                Dim error2 As Decimal = val - nominal
                ' Using the smaller absolute value as the error (handling potential input variations)
    ...
    Private Sub LoadInstrumentDetails(controlNo As String)
        Try
            ...
            Next
            If Not found Then
                _instrumentName = "" : ComboBox2.Text = "" : TextBoxColor.Text = "" : TextBox21.Text = "25-50 mm" : TextBox21.SelectedIndex = -1
            End If
        Catch ex As Exception
        End Try
    End Sub
    ...
    Private Sub LoadDynamicLC()
        Try
            Dim dt = MySqlCls.ReadDatatable("SELECT DISTINCT LC FROM thread_pitch_micrometer_50 WHERE RowType='MASTER' ORDER BY LC ASC")
    ...
    Private Sub ComboBox5_SelectionChanged(sender As Object, e As SelectionChangedEventArgs) Handles ComboBox5.SelectionChanged
        If ComboBox5.SelectedItem IsNot Nothing Then
            ...
            _masterLimits = MySqlCls.GetThreadPitchMicrometer50MasterLimits(selectedLC)
    ...
    Private Sub Button2_Click(sender As Object, e As RoutedEventArgs) Handles Button2.Click
        ...
        If IsEditMode Then
            saved = MySqlCls.UpdateThreadPitchMicrometer50(
                ...
                calibDateToSave)
        Else
            saved = MySqlCls.InsertThreadPitchMicrometer50(
        ...
        For i = 0 To 2
            If i < tbs.Count Then
                ...
                If Decimal.TryParse(tbs(i).Text, val) Then
                    ' Error handling logic same as in TrialTextChanged
    ...
    Private Sub CalculateTMU()
        Try
            ...
            ' Only include valid trials in TMU
            For Each r In nominalRanges
                Dim tbs = GetTextBoxesForRange(r)
                Dim validVals = tbs.Where(Function(t) Decimal.TryParse(t.Text, Nothing)).Select(Function(t) Decimal.Parse(t.Text)).ToArray()
                If validVals.Length >= 2 Then
                    sumSqMU += (calcStdev(validVals) / Math.Sqrt(validVals.Length)) ^ 2 + (masterU ^ 2)
                End If
            Next
