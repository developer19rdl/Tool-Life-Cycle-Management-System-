...
Public Class ThreadPitchGaugeCalibration
    Private MySqlCls As New MySQLClass()
    Private _masterLimits As DataRow = Nothing
    Private _instrumentName As String = "Thread Pitch Gauge"
    ...
    Private _observationRows As New List(Of ThreadPitchObservationRow)()
    Private _navTextBoxes As New List(Of TextBox)()


    Public Class ThreadPitchObservationRow
        Public SizeBox As TextBox
        Public MinBox As TextBox
        Public MaxBox As TextBox
        Public PitchBoxes As TextBox()
        Public AngleBoxes As TextBox()
    ...
    Private Sub Window_Loaded(sender As Object, e As RoutedEventArgs) Handles Me.Loaded
        ...
        Try
            ComboLC.Items.Clear()
            Dim dt = MySqlCls.ReadDatatable("SELECT DISTINCT LC FROM thread_pitch_gauge_calibration WHERE RowType='MASTER' ORDER BY LC ASC")
        ...
        If Not IsEditMode Then
            ...
            If String.IsNullOrEmpty(TxtTimeIn.Text) Then
    ...
    Private Sub TxtCalibrationDate_SelectedDateChanged(sender As Object, e As SelectionChangedEventArgs) Handles TxtCalibrationDate.SelectedDateChanged
        ...
        If TxtCalibrationDate.SelectedDate.HasValue Then
            ...
            If env IsNot Nothing Then
                TxtTemp.Text = env.Temperature
                TxtHumidity.Text = env.Humidity
                TxtTimeIn.Text = env.TimeIn
            Else
                TxtTemp.Text = ""
                TxtHumidity.Text = ""
                TxtTimeIn.Text = ""
        ...
        Try
            ...
            For Each r As DataRow In dtSettings.Rows
                ...
                If dt.Rows.Count > 0 Then
                    ...
                    _instrumentName = If(_category.Equals("Instrument", StringComparison.OrdinalIgnoreCase), If(row.Table.Columns.Contains("InstrumentName"), row("InstrumentName").ToString(), "Thread Pitch Gauge"), If(row.Table.Columns.Contains("GaugeName"), row("GaugeName").ToString(), "Thread Pitch Gauge"))
    ...
    Private Sub FetchMasterLimits()
        Try
            _masterLimits = MySqlCls.GetThreadPitchGaugeMasterLimits(_lc)
    ...
    Private Sub PopulateCalibrationData()
        ...
        _isPopulating = True
        Try
            Dim masterRow = MySqlCls.GetThreadPitchGaugeMasterData(TxtControlNo.Text.Trim(), TargetCycle)
            If masterRow IsNot Nothing Then
                ...
                If Not IsDBNull(masterRow("Status")) Then ComboJudgement.Text = masterRow("Status").ToString()

                Dim dtRecords = MySqlCls.GetThreadPitchGaugeRecordData(TxtControlNo.Text.Trim(), TargetCycle)
                ...
                For idx As Integer = 0 To dtRecords.Rows.Count - 1
                    ...
                    If Not IsDBNull(recordRow("Pitch_Obs_1")) Then rowObj.PitchBoxes(0).Text = recordRow("Pitch_Obs_1").ToString()
                    If Not IsDBNull(recordRow("Pitch_Obs_2")) Then rowObj.PitchBoxes(1).Text = recordRow("Pitch_Obs_2").ToString()
                    If Not IsDBNull(recordRow("Pitch_Obs_3")) Then rowObj.PitchBoxes(2).Text = recordRow("Pitch_Obs_3").ToString()

                    If Not IsDBNull(recordRow("Angle_Obs_1")) Then rowObj.AngleBoxes(0).Text = recordRow("Angle_Obs_1").ToString()
                    If Not IsDBNull(recordRow("Angle_Obs_2")) Then rowObj.AngleBoxes(1).Text = recordRow("Angle_Obs_2").ToString()
                    If Not IsDBNull(recordRow("Angle_Obs_3")) Then rowObj.AngleBoxes(2).Text = recordRow("Angle_Obs_3").ToString()
                Next
    ...
    Private Sub ValidateRowBoxes(row As ThreadPitchObservationRow)
        If _isPopulating Then Return
        For Each tb In row.PitchBoxes
            ValidateBox(tb, row.MinBox.Text, row.MaxBox.Text)
        Next
        For Each tb In row.AngleBoxes
            ValidateBox(tb, row.MinBox.Text, row.MaxBox.Text)
        Next
    End Sub
    ...
    Private Sub UpdateJudgement()
        ...
        For Each row In _observationRows
            ...
            For Each tb In row.PitchBoxes
                ...
                ElseIf Not String.IsNullOrWhiteSpace(tb.Text) Then
                    isAnyNG = True
                End If
            Next
    ...
    Private Sub BtnConfigureSizes_Click(sender As Object, e As RoutedEventArgs)
        ...
        Dim lblPrompt As New TextBlock() With {
            .Text = "Enter number of pitches to measure:",
    ...
    Private Sub GenerateObservationRows(numRows As Integer)
        ...
        For i As Integer = 1 To numRows
            ...
            ' Create Pitch Obs 1, 2, 3
            Dim pitchBoxes(2) As TextBox
            For j As Integer = 0 To 2
                pitchBoxes(j) = New TextBox() With {.HorizontalAlignment = HorizontalAlignment.Stretch, .Margin = New Thickness(2)}
                Grid.SetColumn(pitchBoxes(j), 3 + j)
                rowGrid.Children.Add(pitchBoxes(j))
            Next

            ' Create Angle Obs 1, 2, 3
            Dim angleBoxes(2) As TextBox
            For j As Integer = 0 To 2
                angleBoxes(j) = New TextBox() With {.HorizontalAlignment = HorizontalAlignment.Stretch, .Margin = New Thickness(2)}
                Grid.SetColumn(angleBoxes(j), 6 + j)
                rowGrid.Children.Add(angleBoxes(j))
            Next

            Dim rowObj As New ThreadPitchObservationRow() With {
                .SizeBox = txtSize,
                .MinBox = txtMin,
                .MaxBox = txtMax,
                .PitchBoxes = pitchBoxes,
                .AngleBoxes = angleBoxes,
                .RowGrid = rowGrid
            }
            ...
            For j As Integer = 0 To 2
                AddHandler pitchBoxes(j).TextChanged, Sub(sender, args)
                                                           ValidateBox(TryCast(sender, TextBox), txtMin.Text, txtMax.Text)
                                                       End Sub
                AddHandler angleBoxes(j).TextChanged, Sub(sender, args)
                                                           ValidateBox(TryCast(sender, TextBox), txtMin.Text, txtMax.Text)
                                                       End Sub
            Next
            ...
            For Each tb In pitchBoxes
                _navTextBoxes.Add(tb)
            Next
            For Each tb In angleBoxes
                _navTextBoxes.Add(tb)
            Next
    ...
    Private Sub BtnUpdate_Click(sender As Object, e As RoutedEventArgs)
        ...
        MySqlCls.DeleteThreadPitchGaugeCalibration(controlNo, targetCyc)

        ' Insert MASTER row
        Dim success = MySqlCls.InsertThreadPitchGaugeMaster(controlNo, targetCyc, calibDate, type, size, lc, color, location, temp, humidity, tmu, mu, tempMin, tempMax, humMin, humMax, status, remark, ProjectSettings.Current.Username, timeIn, timeOut, totalTime)
        ...
        If success Then
            ...
            For Each row In _observationRows
                ...
                Dim p1 As Decimal? = Nothing
                Dim p2 As Decimal? = Nothing
                Dim p3 As Decimal? = Nothing
                If Decimal.TryParse(row.PitchBoxes(0).Text, decVal) Then p1 = decVal
                If Decimal.TryParse(row.PitchBoxes(1).Text, decVal) Then p2 = decVal
                If Decimal.TryParse(row.PitchBoxes(2).Text, decVal) Then p3 = decVal

                Dim a1 As Decimal? = Nothing
                Dim a2 As Decimal? = Nothing
                Dim a3 As Decimal? = Nothing
                If Decimal.TryParse(row.AngleBoxes(0).Text, decVal) Then a1 = decVal
                If Decimal.TryParse(row.AngleBoxes(1).Text, decVal) Then a2 = decVal
                If Decimal.TryParse(row.AngleBoxes(2).Text, decVal) Then a3 = decVal

                ' Check judgement for this specific pitch/row
                Dim rowJudgement = "OK"
                For Each tb In row.PitchBoxes
                    Dim v As Decimal
                ...
                MySqlCls.InsertThreadPitchGaugeRecord(controlNo, targetCyc, calibDate, type, size, lc, color, location, measRange, minLimit, maxLimit, p1, p2, p3, a1, a2, a3, rowJudgement, "", ProjectSettings.Current.Username)
            Next
    ...
    Private Sub CalculateTMU()
        Try
            ...
            ' Row-by-Row TMU calculation based on Pitch and Angle Readings
            Dim sumSqMU As Decimal = 0
            Dim hasValidRow As Boolean = False

            For Each row In _observationRows
                ' Pitch Readings
                Dim pitchVals As New List(Of Decimal)()
                For Each tb In row.PitchBoxes
                    Dim val As Decimal
                    If Decimal.TryParse(tb.Text, val) Then pitchVals.Add(val)
                Next

                If pitchVals.Count > 0 Then
                    ' 1. Calculate Standard Deviation in Microns (for precision)
                    Dim microns = pitchVals.Select(Function(v) v * 1000D).ToArray()
                    Dim stdev_microns As Decimal = If(pitchVals.Count > 1, calcStdev(microns), 0)
                    
                    ' 2. Convert back to mm and calculate Sigma B1
                    Dim stdev_mm As Decimal = stdev_microns / 1000D
                    Dim sigB1_row As Decimal = stdev_mm / Math.Sqrt(pitchVals.Count)
                ...
                ' Angle Readings
                Dim angleVals As New List(Of Decimal)()
                For Each tb In row.AngleBoxes
                    Dim val As Decimal
                    If Decimal.TryParse(tb.Text, val) Then angleVals.Add(val)
                Next

                If angleVals.Count > 0 Then
                    ' 1. Calculate Standard Deviation in Microns (for precision)
                    Dim microns = angleVals.Select(Function(v) v * 1000D).ToArray()
                    Dim stdev_microns As Decimal = If(angleVals.Count > 1, calcStdev(microns), 0)
                    
                    ' 2. Convert back to mm and calculate Sigma B1
                    Dim stdev_mm As Decimal = stdev_microns / 1000D
                    Dim sigB1_row As Decimal = stdev_mm / Math.Sqrt(angleVals.Count)
