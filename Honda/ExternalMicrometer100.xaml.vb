...
Public Class ExternalMicrometer100
    ...
    Private ReadOnly nominalRanges As Decimal() = {
        ...
        77.5, 80.1, 82.7, 85.3, 87.9, 90.0, 92.6, 95.2, 97.8, 100.0
    }
    ...
    Private Sub Window_Loaded(sender As Object, e As RoutedEventArgs) Handles Me.Loaded
        ComboBox1.Text = "External Micrometer 100"
        LoadDynamicLC()
    ...
    Private Sub PopulateCalibrationData()
        Try
            Dim dt = MySqlCls.GetExternalMicrometer100Data(TextBox1.Text.Trim(), TargetCycle)
            If dt.Rows.Count > 0 Then
                ...
                ' Set LC by matching the item (triggers SelectionChanged but _isPopulating guards it)
                ...
                For Each r In nominalRanges
                    ...
                    If tbs.Count > 0 AndAlso tbs(0).IsEnabled Then
                        SetTrialsFromDB(row, $"Obs{r.ToString("0.0").Replace(".", "_")}_", tbs)
                    End If
                Next
    ...
    Private Sub SetGeometricFromDB(row As DataRow, gf As String, tbs As List(Of TextBox))
        For i = 0 To 2
            If i < tbs.Count Then
                Dim colName = If(gf = "Parallel", $"Parallelism_{i + 1}", $"Flatness_{gf}_{i + 1}")
    ...
    Private Sub AdjustVisibilityBySize(sizeStr As String)
        ...
        Try
            _activeMinRange = 0 : _activeMaxRange = 100
    ...
    Private Sub LoadDynamicLC()
        Try
            Dim dt = MySqlCls.ReadDatatable("SELECT DISTINCT LC FROM external_micrometer_100 WHERE RowType='MASTER' ORDER BY LC ASC")
    ...
    Private Sub ComboBox5_SelectionChanged(sender As Object, e As SelectionChangedEventArgs) Handles ComboBox5.SelectionChanged
        ...
        _masterLimits = MySqlCls.GetExternalMicrometer100MasterLimits(selectedLC)
        ...
        ' Skip reset during edit population — fields are populated separately
        If _isPopulating Then Return
    ...
    Private Sub CalculateTMU()
        ...
        Try
            ...
            ' RSS: SQRT(Master1² + Master2² + ...) as per reference formula
            ...
            ' 2. Geometric Repeatability (adds master uncertainty for EACH factor)
            For Each gf In geometricFactors
                Dim vals = GetTextBoxesForGeometric(gf).Select(Function(t) If(Decimal.TryParse(t.Text, Nothing), Decimal.Parse(t.Text), 0D)).ToArray()
                Dim sigmaB1 = calcStdev(vals) / Math.Sqrt(3)
                totalSumSq += (sigmaB1 ^ 2) + (masterU_Geometric ^ 2)
            Next

            ' 3. Accuracy Repeatability (adds master uncertainty for EACH active range)
            For Each r In nominalRanges
                Dim tbs = GetTextBoxesForRange(r)
                If tbs.Count > 0 AndAlso tbs(0).IsEnabled Then
    ...
    Private Sub Button2_Click(sender As Object, e As RoutedEventArgs)
        ...
        If IsEditMode Then
            saved = MySqlCls.UpdateExternalMicrometer100(
                "External Micrometer", TextBox1.Text.Trim(), TargetCycle, TextBox21.Text, ComboBox5.Text, TextBoxColor.Text, ComboBox2.Text, TextBox30.Text, TextBox28.Text, TextBox29.Text,
        ...
        Else
            saved = MySqlCls.InsertExternalMicrometer100(
                "External Micrometer", TextBox1.Text.Trim(), cycleName, TextBox21.Text, ComboBox5.Text, TextBoxColor.Text, ComboBox2.Text, TextBox30.Text, TextBox28.Text, TextBox29.Text,
                anvil, spindle, parallel, obsDict,
                If(String.IsNullOrEmpty(TextBox20.Text), "-", TextBox20.Text), TextBox24.Text, TextBox26.Text, TextBox25.Text, statusVal, TextBox19.Text, calibDate)
        End If
