...
Public Class StraightPlugGaugeCalibration
    Private MySqlCls As New MySQLClass()
    Private _masterLimits As DataRow = Nothing
    Private _instrumentName As String = "Straight Plug Gauge"
    ...
    Private Sub Window_Loaded(sender As Object, e As RoutedEventArgs) Handles Me.Loaded
        ...
        Try
            ComboLC.Items.Clear()
            Dim dt = MySqlCls.ReadDatatable("SELECT DISTINCT LC FROM straight_plug_gauge_calibration WHERE RowType='MASTER' ORDER BY LC ASC")
        ...
        Dim obsFields = {TxtObs1, TxtObs2, TxtObs3, TxtObs4}
        ...
        ' Wire up limit boxes to trigger re-validation
        AddHandler TxtMin.TextChanged, Sub() RevalidateObs()
        AddHandler TxtMax.TextChanged, Sub() RevalidateObs()
        ...
        Try
            ...
            For Each r As DataRow In dtSettings.Rows
                ...
                If dt.Rows.Count > 0 Then
                    ...
                    _instrumentName = If(_category.Equals("Instrument", StringComparison.OrdinalIgnoreCase), If(row.Table.Columns.Contains("InstrumentName"), row("InstrumentName").ToString(), "Straight Plug Gauge"), If(row.Table.Columns.Contains("GaugeName"), row("GaugeName").ToString(), "Straight Plug Gauge"))
    ...
    Private Sub FetchMasterLimits()
        Try
            _masterLimits = MySqlCls.GetStraightPlugGaugeMasterLimits(_lc)
    ...
    Private Sub PopulateCalibrationData()
        ...
        _isPopulating = True
        Try
            Dim dt = MySqlCls.GetStraightPlugGaugeCalibrationData(TxtControlNo.Text.Trim(), TargetCycle)
            If dt.Rows.Count > 0 Then
                Dim dr = dt.Rows(0)
                ...
                ' Gauge details
                If Not IsDBNull(dr("Size")) Then TxtSize.Text = dr("Size").ToString()
    ...
    Private Sub TrialTextChanged(sender As Object, e As TextChangedEventArgs)
        ...
        If Decimal.TryParse(tb.Text, val) Then
            Dim isOK = True
            Dim ul As Decimal = 99999.0
            Dim ll As Decimal = -99999.0
            
            Dim minVal, maxVal As Decimal
            ll = If(Decimal.TryParse(TxtMin.Text, minVal), minVal, -99999.0)
            ul = If(Decimal.TryParse(TxtMax.Text, maxVal), maxVal, 99999.0)
    ...
    Private Sub UpdateJudgement()
        ...
        ' Check observations
        If Not String.IsNullOrWhiteSpace(TxtSize.Text) Then
            Dim minVal, maxVal As Decimal
            If Decimal.TryParse(TxtMin.Text, minVal) AndAlso Decimal.TryParse(TxtMax.Text, maxVal) Then
                Dim obsTbs = {TxtObs1, TxtObs2, TxtObs3, TxtObs4}
                For Each obsTb In obsTbs
                    Dim val As Decimal
    ...
    Private Sub BtnUpdate_Click(sender As Object, e As RoutedEventArgs)
        ...
        Dim locationObs = "Top, Middle, Bottom"
        ...
        Dim dMin, dMax As Decimal
        Dim minLimit As Object = If(Decimal.TryParse(TxtMin.Text, dMin), dMin, DBNull.Value)
        Dim maxLimit As Object = If(Decimal.TryParse(TxtMax.Text, dMax), dMax, DBNull.Value)

        Dim success As Boolean = False
        Dim targetCyc = If(IsEditMode, TargetCycle, cycleName)

        If IsEditMode Then
            success = MySqlCls.UpdateStraightPlugGaugeCalibration(type, controlNo, TargetCycle, size, lc, color, location, temp, humidity, tmu, mu, parameter, locationObs, minLimit, maxLimit, TxtObs1.Text, TxtObs2.Text, TxtObs3.Text, TxtObs4.Text, timeIn, timeOut, totalTime, status, remark, TxtCalibrationDate.SelectedDate)
        Else
            success = MySqlCls.InsertStraightPlugGaugeCalibration(type, controlNo, cycleName, TxtCalibrationDate.SelectedDate, size, lc, color, location, temp, humidity, tmu, mu, parameter, locationObs, minLimit, maxLimit, TxtObs1.Text, TxtObs2.Text, TxtObs3.Text, TxtObs4.Text, timeIn, timeOut, totalTime, status, remark)
        End If
    ...
    Private Sub CalculateTMU()
        Try
            ...
            ' 2. Calculate mu_sq for observations
            ...
            If obsVals.Count = 4 Then
                Dim stdevVal = calcStdev(obsVals.ToArray())
                Dim sigB = stdevVal / Math.Sqrt(4)
                Dim mu_sq = (sigB ^ 2) + (masterU ^ 2)
