...
Public Class GaugeManagementPage
    Inherits Page
    ...
    Public Sub New(Optional typeName As String = "")
        ...
        ' forInventory:=True ensures gauge types like "Plain Plug Gauge" resolve to their
        ' inventory table (plain_plug_gauge) and not the calibration table (plain_plug_gauge_calibration).
    ...
    Private Sub LoadData()
        ...
        Try
            ...
            _totalCount = _mySql.GetFilteredGaugeTotalCount(_tableName, filters, keyword)
            
            Dim offset = (_currentPage - 1) * _pageSize
            Dim dt = _mySql.GetGaugeInventoryData(_tableName, offset, _pageSize, filters, keyword)

            GaugesDataGrid.ItemsSource = dt.DefaultView
            UpdatePaginationUI()
        Catch ex As Exception
    ...
    Private Sub BtnAdd_Click(sender As Object, e As RoutedEventArgs) Handles BtnAdd.Click
        Dim win As New AddEditGaugeWindow(_tableName)
        win.DefaultGaugeName = _typeNameFilter
    ...
    Private Sub BtnEdit_Click(sender As Object, e As RoutedEventArgs) Handles BtnEdit.Click
        If GaugesDataGrid.SelectedItem Is Nothing Then
            MessageBox.Show("Please select a gauge to edit.", "Selection Required", MessageBoxButton.OK, MessageBoxImage.Warning)
            Return
        End If

        Dim rowView As DataRowView = GaugesDataGrid.SelectedItem
        Dim win As New AddEditGaugeWindow(_tableName)
        win.Owner = Window.GetWindow(Me)
    ...
    Private Sub GaugesDataGrid_ContextMenuOpening(sender As Object, e As ContextMenuEventArgs)
        If GaugesDataGrid.SelectedItem Is Nothing Then
            e.Handled = True
            Return
        End If

        Dim selectedRow As DataRowView = DirectCast(GaugesDataGrid.SelectedItem, DataRowView)
    ...
    Private Sub MenuWriteOff_Click(sender As Object, e As RoutedEventArgs)
        If GaugesDataGrid.SelectedItem Is Nothing Then Return
        Dim selectedRow As DataRowView = DirectCast(GaugesDataGrid.SelectedItem, DataRowView)
    ...
    Private Sub MenuMenuView_Click(sender As Object, e As RoutedEventArgs)
        ...
        Dim historyWin As New ItemHistoryWindow(ctrlNo, "Gauge", _tableName)
        historyWin.ShowDialog()
    End Sub

    Private Sub BtnDelete_Click(sender As Object, e As RoutedEventArgs) Handles BtnDelete.Click
        If GaugesDataGrid.SelectedItem Is Nothing Then
            MessageBox.Show("Please select a gauge to delete.", "Selection Required", MessageBoxButton.OK, MessageBoxImage.Warning)
            Return
        End If
                                   ...
                                   Using stream = File.Open(filePath, FileMode.Open, FileAccess.Read)
                                        ...
                                        Using reader
                                           ...
                                           For Each table As DataTable In result.Tables
                                               ...
                                               Dim idxDrg = -1, idxTol = -1, idxTol2 = -1, idxTol3 = -1, idxDD = -1, idxMM = -1, idxYY = -1
                                                   ...
                                                   For c As Integer = 0 To table.Columns.Count - 1
                                                       ...
                                                       If IsMatch(colName, "Maker Name") Then idxMaker = c
                                                       ...
                                                       If IsMatch(colName, "Request no") Then idxReq = c
                                                       If IsMatch(colName, "Gauge Description", "Gauge Name", "Name") Then idxName = c
                                                       If IsMatch(colName, "Model") Then idxModel = c
                                                       If IsMatch(colName, "Line") Then idxLine = c
                                                       If IsMatch(colName, "DRG NO", "DrgNo") Then idxDrg = c
                                                       If IsMatch(colName, "Tol") Then idxTol = c
                                                       If IsMatch(colName, "Tol2") Then idxTol2 = c
                                                       If IsMatch(colName, "Tol3") Then idxTol3 = c
                                                       ...
                                                       If idxDD <> -1 AndAlso idxMM <> -1 AndAlso idxYY <> -1 Then
                                                           ...
                                                           If Not String.IsNullOrWhiteSpace(ddStr) AndAlso Not String.IsNullOrWhiteSpace(mmStr) AndAlso Not String.IsNullOrWhiteSpace(yyStr) Then
                                                               ...
                                                               If Integer.TryParse(ddStr, d) AndAlso Integer.TryParse(mmStr, m) AndAlso Integer.TryParse(yyStr, y) Then
                                                                   ' If year is provided as 2 digits (e.g. 26), convert to 2026. Same for 21st century handling
                                                                   If y < 100 Then
                                                                       y += 2000
                                                                   End If
                                                                   Try
                                                                   ...
                                                                   Catch ex As Exception
                                                                       ' Ignore invalid date
                                                                   End Try
                                                                   dateAdd = $"{d:D2}.{m:D2}.{y:D4}"
                                                               End If
                                                           End If
                                                       End If

                                                   Dim success As Boolean = _mySql.Insertgauge(
                                                       _tableName,
                                                       If(Not String.IsNullOrEmpty(_typeNameFilter), _typeNameFilter, GetVal(row, idxName)),
                                                       "",
                                                       controlNoVal,
                                                       GetVal(row, idxColor),
                                                       GetVal(row, idxDrg),
    ...
    Private Sub BtnDownload_Click(sender As Object, e As RoutedEventArgs) Handles BtnDownload.Click
        Try
            ...
            Dim defaultName As String = "Gauges_Export"
            If Not String.IsNullOrEmpty(_typeNameFilter) Then
            ...
            If saveFileDialog.ShowDialog() = True Then
                Dim view As DataView = DirectCast(GaugesDataGrid.ItemsSource, DataView)
                Dim dt = view.ToTable()
                
                Using sw As New StreamWriter(saveFileDialog.FileName)
                    Dim cols = {"ID", "Date", "ControlNo", "GaugeName", "GaugeDescription", "Color", "CategoryControl", "DrgNo", "Model", "Line", "Size", "Tol", "Tol2", "Tol3", "MakerName", "Section", "Location", "DateofAddition", "RequestNo", "Remark"}
